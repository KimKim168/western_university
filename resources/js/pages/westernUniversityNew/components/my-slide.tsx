                    ...
                    {banners?.map((item) => (
                        <div key={item.id} className="flex-[0_0_100%]">
                            <div className="relative aspect-[21/7] w-full">
                                <img src={`/assets/images/banners/${item.image}`} alt='images' className="h-full w-full object-cover" />
                                ...
                                <div className="absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2 px-4 text-white sm:px-10 md:px-16 lg:px-20">
                                    <div className="max-w-full p-5 px-1 text-center sm:max-w-screen-xl sm:text-left md:max-w-[50%]">
                                        <h3 className="text-sm font-bold sm:text-lg md:text-2xl lg:text-4xl xl:text-5xl font-noto-san-extra-light">{item.title}</h3>
                                        {/* <p className="mt-2 text-xs sm:mt-3 sm:text-sm md:mt-5 md:text-base">{item.short_description}</p> */}
                                        <div className="mt-4 flex flex-col items-center gap-2 sm:flex-row sm:items-start sm:gap-4">
                                            <a
                                                href={item.link}
                                                className="rounded border border-white bg-[#b30e13] px-4 py-2 text-xs text-white hover:bg-transparent hover:text-white sm:px-6 sm:text-base"
                                            >
