...
const MyHeroBottomHistory = () => {
    ...
    const showPrev = () => setCurrentIndex((prev) => (prev === 0 ? vision?.images?.length - 1 : prev - 1));
    const showNext = () => setCurrentIndex((prev) => (prev === vision?.images?.length - 1 ? 0 : prev + 1));
            ...
            <div className="mx-auto max-w-screen-xl px-6 xl:px-0">
                <div className="mt-4 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {vision?.images?.map((img, index) => (
                        ...
                        >
                            <img className="h-full w-full object-cover" src={`/assets/images/pages/${img.image}`} />
                        </div>
                    ))}
                </div>
                    ...
                    <div>
                        ...
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                                __html: locale === 'kh' ? (vision?.short_description_kh ?? vision?.short_description) : vision?.short_description,
                            }}
                        />
                    </div>
                </div>
