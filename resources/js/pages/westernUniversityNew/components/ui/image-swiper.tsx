...
export function ImageSwiper({imageSrc, images, className, ...props }: ImageSwiperProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className=" flex cursor-grab items-center rounded-[inherit] active:cursor-grabbing">
        {images.map((item, i) => {
          return (
            <motion.div
              key={i}
              ...
              <img src={`${imageSrc}/${item.image}`} className="pointer-events-none h-full w-full object-cover" />
            </motion.div>
          )
        })}
      </motion.div>
