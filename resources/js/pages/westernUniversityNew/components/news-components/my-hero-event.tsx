            ...
            <div className={`flex flex-col-reverse lg:flex-row ${reverse ? 'lg:flex-row-reverse' : ''} mx-auto max-w-screen-2xl items-center`}>
                ...
                <div className="w-full px-6 sm:px-10 lg:px-20 lg:w-1/2">
                    <div className="prose dark:prose-invert ck-content lg:text-left">
                        <h2 className={`my-6 pt-5 text-2xl font-bold text-red-700 sm:text-3xl lg:my-0 lg:mb-4 ${fontClass}`}>{title}</h2>
                        <div
...
const MyHeroEvent = ({ children }: { children?: any }) => {
    ...
    return (
        <section>
            {children?.map((activity:any, index:any) => (
                <ActivityItem
                    id={`event_id${activity?.id}`} 
                    key={activity?.id}
                    title={locale === 'kh' ? (activity?.title_kh ?? activity.title) : activity.title}
                    short_description={locale === 'kh' ? (activity?.short_description_kh ?? activity?.short_description) : activity?.short_description}
