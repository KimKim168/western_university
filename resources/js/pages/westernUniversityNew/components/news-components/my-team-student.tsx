...
const MyTeamStudent = () => {
  const { gradeLevelLeaders, locale } = usePage().props;
  ...
  return (
    <div className="flex flex-col justify-center px-6 lg:px-0 lg:px- max-w-screen-xl mx-auto">
      <h2 className={`mt-3 text-3xl md:text-4xl font-bold tracking-tighter border-l-4  border-red-700 pl-2 ${fontClass}`}>
        {locale === 'kh' ? (gradeLevelLeaders?.title_kh ?? gradeLevelLeaders?.title) : gradeLevelLeaders?.title}
      </h2>
      <div className="mt-4 text-base sm:text-lg prose dark:prose-invert ck-content whitespace-pre-line" dangerouslySetInnerHTML={ {__html:locale === 'kh' ? (gradeLevelLeaders?.short_description_kh ?? gradeLevelLeaders?.short_description) : gradeLevelLeaders?.short_description}}/>
        ...
        {gradeLevelLeaders?.children?.map((member) => (
          <div key={member.id}>
            <img
              ...
              height={120}
            />
            <h3 className="mt-4 text-lg xl:text-xl font-semibold prose dark:prose-invert ck-content">
